...
@stop
  ...
  <section class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1>Creaci&oacute;n de Fuentes de Financiamiento</h1>
            </div>
            <div class="col-sm-2">
    ...
    <div class="container-fluid">
        <div class="card card-warning">
          <div class="card-header">
            <h3 class="card-title">Fuentes de financimiento</h3>
...
<!-- modal Agregar fuente de financimiento -->
<div class="modal fade" id="modalAgregar">
        <div class="modal-dialog modal-xl">
          <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">Agregar nueva Fuente de financiamiento</h4>
   ...
   <!-- modal editar fuente de financiamiento -->
   <div class="modal fade" id="modalEditar">
        <div class="modal-dialog modal-xl">
          <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">Editar Fuente de financiamiento</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
              ...
              <div class="card-body">
                <div class="row">  
                  ...
                  <div class="form-group">
                    ...
                    <div class="form-group">
                      <label>Nombre de la Fuente de Financiamiento:</label>
  ...
  <div class="modal fade" id="modalEliminar">
      <div class="modal-dialog">
            <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">Eliminar Fuente de financiamiento</h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
              </button>
            </div>
            ...
            <div class="modal-footer justify-content-between">
              <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>             
              <button class="btn btn-danger" id="btnBorrar" type="button" onclick="borrarfuentef()">Borrar</button>
          </div>
        </div>      
      </div>        
  </div>
...
// abre el modal para editar las fuentes de financiamiento
function abrirModalEditar(id){
  document.getElementById("formularioU").reset();   
  spinHandle = loadingOverlay().activate();
  axios.post('/admin/get_fuentef',{'id': id })
      .then((response) => {	
        loadingOverlay().cancel(spinHandle); // cerrar loading
        if(response.data.success = 1){
          $('#modalEditar').modal('show');
          $('#idU').val(response.data.fuentef.id);
          $('#nombre').val(response.data.fuentef.nombre);    
          $('#codigo').val(response.data.fuentef.codigo);   
        }else{ 
          toastr.error('Error', 'Fuente de financiamiento no encontrada'); 
        }
      })
      .catch((error) => {
        loadingOverlay().cancel(spinHandle); // cerrar loading
...
function enviarModalEditar(){
      ...
      axios.post('/admin/update_fuentef', formData, {  
       })
       .then((response) => {	
         loadingOverlay().cancel(spinHandle); // cerrar loading            
    ...
    // guardar nueva fuente de financ.
    function enviarModalAgregar(){
      ...
      axios.post('/admin/add_fuentef', formData, {  
       })
       .then((response) => {	
         loadingOverlay().cancel(spinHandle); // cerrar loading            
...
// mensaje cuando se guardan las fuentes de financiamiento
function mensajeResponse2(valor){
  if(valor.data.success == 1){
    toastr.success('Guardado', 'Se han guardado los cambios en la fuente de financiamiento!');
...
function mensajeResponse1(valor){
  if(valor.data.success == 1){
    toastr.success('Guardado', 'Se ha guardado la nueva fuente de financiamiento!');
...
// abre el modal para eliminar la fuente
function abrirModalEliminar(id){     
  $('#modalEliminar').modal('show');
  $('#idD').val(id);    
}

// enviar peticion para borrar la fuente de financiamiento
function borrarfuentef(){
  id = document.getElementById("idD").value;
  spinHandle = loadingOverlay().activate(); // mostrar loading

  axios.post('/admin/delete_fuentef',{
    'id': id  
      })
      .then((response) => {	
        loadingOverlay().cancel(spinHandle); // cerrar loading
        ...
        if(response.data.success == 1){
          toastr.success('Fuente de financiamiento Eliminada!')
          $('#modalEliminar').modal('hide');   
        }else{
          toastr.error('Error', 'No se pudo eliminar la fuente de financiamiento');  
        }           
      })
      .catch((error) => {
        loadingOverlay().cancel(spinHandle); // cerrar loading   
